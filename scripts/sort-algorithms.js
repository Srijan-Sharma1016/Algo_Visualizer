...
class sortAlgorithms {
    ...
    // BUBBLE SORT
    BubbleSort = async () => {
        const totalIterations = this.size - 1;
        for (let i = 0; i < this.size - 1; ++i) {
            ...
            this.updateProgress(i + 1, totalIterations);
        }
    ...
    // SELECTION SORT
    SelectionSort = async () => {
        const totalIterations = this.size;
    ...
    MergeSort = async () => {
        ...
        this.totalMergeOps = this.size * Math.ceil(Math.log2(this.size));
    ...
    Merge = async (start, mid, end) => {
        ...
        for (let c = start, point = 0; c <= end && point < newList.length;
            ...
            this.mergeProgress++;
            if (this.totalMergeOps > 0) {
                const progress = Math.min(95, Math.round((this.mergeProgress / this.totalMergeOps) * 95));
    ...
    // QUICK SORT
    QuickSort = async () => {
        this.quickProgress = 0;
        this.totalQuickOps = this.size * Math.ceil(Math.log2(this.size));
    ...
    Partition = async (start, end) => {
        ...
        for (let c = start; c < end; ++c) {
            ...
            this.quickProgress++;
            if (this.totalQuickOps > 0) {
                const progress = Math.min(95, Math.round((this.quickProgress / this.totalQuickOps) * 95));
                this.updateProgress(progress, 100);
            }
        }
